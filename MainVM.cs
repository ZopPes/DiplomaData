...
namespace DiplomaData
{
    ...
    internal class MainVM : peremlog, IDisposable
    {
        ...
        public MainVM()
        {
            ...
            Report = new TabReport((q, e) => q.Where(w => (w.Student_rus.Имя + " " + w.Student_rus.Отчество + " " + w.Student_rus.Фамилия+" "+w.Thesis_rus.Название_темы).Contains(e)), DiplomaData.Diplom_rus, "Отчет по защите диплома(?)");
            Report.AddFilterList("Группа:", Groups, g => d => d.Student_rus.Group_rus == g);
            Report.AddFilterList("Рукаводитель:", Lecturers, l => d => d.Lecturer_rus == l);
            ...
            UpdateData = new lamdaCommand(() =>
            {
                try
                {
                    ...
                    DiplomaData.Refresh(RefreshMode.OverwriteCurrentValues, DiplomaData.Thesis_rus);
